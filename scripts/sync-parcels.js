...
function slugify(value) {
  return String(value || "property").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "property";
}
...
function parcelSlug(parcelId, address) {
  return `${String(parcelId).toLowerCase()}-${slugify(address)}`;
}
function streetFromAddress(address) {
  const cleaned = String(address || "").replace(/^\d+[a-z]?\s+/i, "").trim();
  return cleaned || null;
}
...
function normalize(feature) {
  ...
  const streetName = a.STREETNAME || a.street_name || streetFromAddress(address);
  const platName = a.PLAT || a.plat_name || a.SUBDIVISION || null;
